...
#### 1. 面板样式切换
- **无边框**: 无边框，保留可调透明度背景
- **默认边框**: 继续使用当前的系统默认框体边缘

#### 2. 背景透明度保留
- **最小样式兼容**: 即使使用无边框样式，仍然可以调节背景透明度
- **视觉效果**: 可保留轻量的毛玻璃/磨砂感面板效果

#### 3. 设置界面更新
- **交互简化**: 用“面板样式”取代单独的背景/边框开关
- **配置更清晰**: 外观逻辑改成样式驱动，而不是多个开关组合

### 版本历史总结

| 版本 | 发布日期 | 主要更新 |
|------|----------|----------|
| 2.4.0 | 2026-03-21 | 面板样式切换、保留无边框透明背景、下拉选择交互优化 |
| 2.3.0 | 2026-03-21 | 固定紧凑宽度、行距调节、背景开关、边框开关 |
| 2.2.0 | 2026-03-21 | 双列布局、字体大小调节、背景透明度调节、主属性识别优化 |
| 2.1.0 | 2026-03-15 | 数值统一白色显示，移除属性提升变绿提示 |
...
#### 1. 布局调节
- **固定基础宽度**: 面板基础宽度固定为当前较合适的紧凑值
- **行距调节**: 可根据个人习惯调整属性行之间的疏密

#### 2. 外观开关
- **背景显示开关**: 可隐藏背景，仅保留文本内容
- **边框显示开关**: 可隐藏边框，获得更轻量的视觉效果

#### 3. 设置界面扩展
- **外观配置增强**: 字体大小、背景透明度、行距集中在同一块管理
- **显示配置补充**: 背景和边框可以独立开关
- **窗口尺寸修正**: 扩大设置窗口高度，避免底部按钮超出框体
- **标题位置修正**: 标题上移，避免压到标题栏边缘
...
#### 1. 主面板双列布局
- **布局调整**: 属性标签固定在左侧，数值固定在右侧
- **显示效果**: 百分比和数字统一右对齐，阅读更整齐
- **适用范围**: 装等、主属性、绿字属性、移速和额外属性

#### 2. 外观设置
- **字体大小调节**: 可在设置界面中直接调大或调小
- **背景透明度调节**: 可根据界面风格调整面板背景深浅
- **即时生效**: 修改后立即刷新面板，无需重载

#### 3. 主属性识别补充
- **专精识别优化**: 修正圣骑士、武僧、德鲁伊治疗/敏捷专精的主属性判断
- **显示准确性**: 减少治疗专精被误判为力量或敏捷的情况

#### 4. 设置界面结构更新
- **新增模块**: 增加“外观”分区
- **保留原能力**: 锁定框体、额外属性开关、重置位置仍然保留
...
#### 1. 数值显示样式
- **功能描述**: 属性数值统一使用白色显示
- **适用范围**: 包括装等、主属性和百分比类属性
- **显示规则**: 不再根据属性变化动态切换数值颜色

#### 2. 技术改进
- 简化 `UpdateStat()` 函数，移除属性变化颜色判断
- 删除不再需要的数值比较与颜色状态缓存逻辑
- 数值文本统一按白色格式输出
